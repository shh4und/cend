...
import numpy as np
import logging
from swc import SWCFile  # Assuming SWCFile class exists as in your project
...
class Graph:
    ...
    def _create_graph_from_root(self) -> None:
        ...
        self.logger.info(f"Graph created with {self.graph.number_of_nodes()} nodes and {self.graph.number_of_edges()} edges.")
    ...
    def prune_mst_by_length(self, length_threshold: int):
        ...
        """
        if not self.mst:
            self.logger.warning("MST must be calculated before pruning. Call `calculate_mst()` first.")
            return
    ...
    def validate_and_set_root(self, original_root: Tuple[int, int, int]) -> None:
        ...
        self.logger.warning(f"Original root {original_root} not found in skeleton. Finding the closest node...")
        
        nodes = np.array(list(self.graph.nodes()))
        # Calculate squared Euclidean distance from all nodes to the original root
        distances_sq = np.sum((nodes - np.array(original_root))**2, axis=1) # BUG FIX: Added axis=1
