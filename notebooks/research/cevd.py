...
class VesselCenterlineExtractor:
    def __init__(
        ...
        self.sigmas = np.arange(self.sigma_min, self.sigma_max + self.sigma_step, self.sigma_step)
    ...
    def local_hessian(self, point, sigma):
        ...
        if (
            local_volume_slice.size == 0
            or local_volume_slice.shape[0] < min_dim_size
            or local_volume_slice.shape[1] < min_dim_size
            or local_volume_slice.shape[2] < min_dim_size
        ):
            self.logger.debug(
        ...
        H_comp = {}
        H_comp["xx"] = ndi.gaussian_filter(
            local_volume_slice, sigma=sigma, order=[0, 0, 2], mode="reflect"
        )
        H_comp["yy"] = ndi.gaussian_filter(
            local_volume_slice, sigma=sigma, order=[0, 2, 0], mode="reflect"
        )
        H_comp["zz"] = ndi.gaussian_filter(
            local_volume_slice, sigma=sigma, order=[2, 0, 0], mode="reflect"
        )
        H_comp["xy"] = ndi.gaussian_filter(
            local_volume_slice, sigma=sigma, order=[0, 1, 1], mode="reflect"
        )  # d/dy d/dx
        H_comp["xz"] = ndi.gaussian_filter(
            local_volume_slice, sigma=sigma, order=[1, 0, 1], mode="reflect"
        )  # d/dz d/dx
        H_comp["yz"] = ndi.gaussian_filter(
            local_volume_slice, sigma=sigma, order=[1, 1, 0], mode="reflect"
        )  # d/dz d/dy

        lz, ly, lx = z_global - z_min, y_global - y_min, x_global - x_min

        if not (
            0 <= lz < H_comp["xx"].shape[0]
            and 0 <= ly < H_comp["xx"].shape[1]
            and 0 <= lx < H_comp["xx"].shape[2]
        ):
            self.logger.warning(
    ...
    def compute_eigenvalues(self, point, sigma):
        ...
        try:
            ...
            self.logger.warning(
                f"Decomposição de autovalores falhou para o ponto {point}, sigma {sigma}: {e}"
            )
            return None, None
    ...
    def _find_multiple_peaks_in_cs(
        self,
        cs_center_point,
        v2_cs,
        v3_cs,
        sigma_at_cs,
        min_peak_dist_factor=0.5,
        peak_response_thresh_factor=0.75,
    ):
        ...
        threshold_abs = max(
            self.vessel_threshold * 1.15, max_grid_response * peak_response_thresh_factor
        )
        min_distance_pixels = max(1, int(self.search_radius * min_peak_dist_factor))

        peak_coords_in_grid = peak_local_max(
            response_grid,
    ...
    def find_center_in_cross_section(
        self,
        center_of_cs_plane,
        v2,
        v3,
        best_sigma_at_Pc=None,
        max_response_at_Pc=None,
        radius=None,
    ):
        """
        ...
        if max_response_point is None:
            self.logger.debug(
                f"Nenhum centro encontrado na seção transversal de {center_of_cs_plane}"
            )
    ...
    def extract_centerline(self, seed_point, max_steps=1000, at_step_log=100):
        ...
        if eigenvectors_P0 is None:
            self.logger.warning(
                f"Não foi possível calcular autovalores para a semente inicial {current_P}. Parando."
            )
            return centerline

        v2_P0, v3_P0 = eigenvectors_P0[:, 1], eigenvectors_P0[:, 2]

        current_C = self.find_center_in_cross_section(
            current_P, v2_P0, v3_P0, sigma_P0, response_P0
        )

        if current_C is None:
            self.logger.warning(
                f"Não foi possível encontrar o centro inicial C0 a partir da semente P0={current_P}. Usando P0 como C0."
            )
            current_C = current_P
            response_C0, _ = self.get_multiscale_response(current_C)
            if response_C0 < self.vessel_threshold:
                self.logger.warning(
                    f"Fallback C0={current_C} (de P0) está abaixo do limiar. Parando."
                )
                return centerline
        ...
        for step_num in range(max_steps):
            ...
            next_C = self.find_center_in_cross_section(
                next_P, v2_Pk_plus_1, v3_Pk_plus_1, sigma_Pk_plus_1, response_Pk_plus_1
            )

            if next_C is None:
                self.logger.info(
                    f"Não foi possível encontrar o centro Ck+1 a partir de Pk+1={next_P}. Terminando."
                )
                break

            if np.array_equal(next_C, current_C):
                self.logger.info(
                    f"Novo centro Ck+1={next_C} é o mesmo que Ck={current_C}. Terminando para evitar loop."
                )
                break
            ...
            if step_num % at_step_log == 0 and step_num > 0:  # Log a cada n passos
                self.logger.info(
                    f"No passo: {step_num}, próximo_C: {next_C}, cache_size: {len(self.eigenvalue_cache)}"
                )
                ...
                for ni in [-1, 0, 1]:
                    for nj in [-1, 0, 1]:
                        if ni == 0 and nj == 0:
                            continue
                        is_peak = is_peak and (
                            responses_smoothed[i, j] > responses_smoothed[i + ni, j + nj]
                        )
    ...
    def extract_vessel_tree(
        self,
        seed_point,
        max_branches=20,
        max_branch_length_limit=100,
        ...
        while seed_queue and branch_id < max_branches:
            ...
            self.logger.info(
                f" Processing branch {branch_id}, seed: {current_seed}, parent: {parent_id}"
            )
            ...
            # Extract centerline from this seed
                ...
                centerline = self.extract_centerline(
                    current_seed, max_steps=max_branch_length_limit
                )

            if len(centerline) >= min_branch_length_limit:  # Ensure we found a valid centerline
                vessel_tree[branch_id] = {"centerline": centerline, "parent": parent_id}
                self.all_tree_points.update(centerline)
                ...
                self.logger.info(
                    f" Branch {branch_id}: checking {len(check_points)} points for bifurcations"
                )
                ...
                for point in check_points:
                    ...
                    if len(peaks) > 1:
                        self.logger.info(
                            f" Bifurcation detected at {point} with {len(peaks)} peaks"
                        )

                        for peak in peaks:
                            # Skip peaks too close to any tree point
                            if not any(
                                np.linalg.norm(np.array(peak) - np.array(p)) < self.step_size * 0.9
                                for p in self.all_tree_points
                            ):
                                self.logger.info(
                                    f" Adding new seed at {peak} from branch {branch_id}"
                                )
        ...
        end_time = time.time()
        self.logger.info(
            f" Vessel tree extraction complete: {branch_id} branches, {(end_time - start_time):.2f}s"
        )
